...
const Navbar = () => {
  ...
  const links = [
    ...
    { title: "Why PF", path: "#" }, // This will be the dropdown trigger
        ...
        <div className="hidden lg:flex gap-6 w-full justify-start">
          {links.map((item) =>
            item.title === "Why PF" ? (
              ...
              >
                <span className="flex items-center cursor-pointer px-4 py-2 rounded-xl transition-all duration-300 hover:bg-gray-200 hover:text-purple-600">
                  {item.title}
                </span>

                {/* Dropdown Menu */}
                <div
                  className={`absolute left-0 bg-white shadow-lg rounded-xl mt-2 w-48 z-50 transition-all duration-500 ${
                    dropdownVisible
                      ? "opacity-100 visible"
                      : "opacity-0 invisible"
                  }`}
                  onMouseEnter={() => setDropdownVisible(true)} // Keep dropdown visible when hovering over it
                  onMouseLeave={() => setDropdownVisible(false)} // Hide dropdown when mouse leaves the dropdown
                >
                  <span
                    className="block cursor-pointer px-4 py-2 hover:bg-gray-200 hover:text-purple-600"
                    onClick={() => handleNavigation("/whyplanetfitness")} // Use navigate here
                  >
                    Why PlanetFitness
                  </span>
                  <span
                    className="block cursor-pointer px-4 py-2 hover:bg-gray-200 hover:text-purple-600"
                    onClick={() => handleNavigation("/about")} // Use navigate here
                  >
                    About
                  </span>
                  <span
                    className="block cursor-pointer px-4 py-2 hover:bg-gray-200 hover:text-purple-600"
                    onClick={() => handleNavigation("/perks")} // Use navigate here
                  >
                    Perks
                  </span>
                </div>
